...
uint8_t disableSteerSwitchAutomatic = 0; // Disable automatic steer switch with PTO speed/ hydraulic
...
void autosteerLoop()
{
	...
	if (currentTime - autsteerLastTime >= LOOP_TIME)
	{
		...
		else if (steerConfig.SteerButton == 1) // steer Button momentary
		{
			if (steerBtn)
			{
				timeSteerBtnPressed += LOOP_TIME;
				if (timeSteerBtnPressed >= 10000) // Steer button pressed for 10s
				{
					disableSteerSwitchAutomatic = !disableSteerSwitchAutomatic;
					Serial.println("disableSteerSwitchAutomatic new value= " + String(disableSteerSwitchAutomatic));
					timeSteerBtnPressed = -30000; // Not nice solution that it is toggled again by pressing the button once
				}
			}
			else
			{
				timeSteerBtnPressed = 0;
			}

			// Steer switch
			if (currentState == 1) // Automatic steering disabled
			{
				if ((steerBtn && !steerBtn_last) || (!disableSteerSwitchAutomatic && ((!lifted && lifted_last) || (!ptoOff && ptoOff_last)))) // Switch on
				{
			...
			{
				if ((steerBtn && !steerBtn_last) || (!disableSteerSwitchAutomatic && ((lifted && !lifted_last) || (ptoOff && !ptoOff_last)))) // Switch off
				{
			...
			if ((!lifted && lifted_last) || (!ptoOff && ptoOff_last)) // Switch on
			{
				workSwitch = 1;   // 1=Work
			}
			if ((lifted && !lifted_last) || (ptoOff && !ptoOff_last)) // Switch off
			{
